...
async function showPosition(position) {
  ...
  document.getElementById("stadt").innerText = document.getElementById(
    "input"
  ).value = data.name;
...
async function map() {
  ...
  const attribution =
    '&copy; <a href = https://www.openstreetmap.org/copyright">OpenStrettMap</a> contributors';
  const tileURL = "https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png";
  const tiles = L.tileLayer(tileURL, { attribution });
  tiles.addTo(mymap);
  L.marker([data.coord.lat, data.coord.lon])
    .addTo(mymap)
    .bindPopup("Hier sind es " + Math.round(data.main.temp * 10) / 10 + "°C")
    .openPopup();
